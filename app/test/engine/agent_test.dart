...
''',
  ...
  test('自定义脚本技能：支持可选方法调用 fallback', () async {
    engine.saveCustomAgentSkill(
      id: 'custom_script_optional_method_call_runtime',
      name: '可选方法调用脚本运行时',
      description: '验证自定义技能兼容模型常写的 value?.trim?.() ?? fallback。',
      script: r'''
const labels = args.assets
  .map(asset => asset.name?.trim?.() ?? '未命名')
  .join('、');
...
''',
    ...
    gateway.turns = [
      AgentTurnResult.tool('custom_script_optional_method_call_runtime', const {
        'assets': [
          {'name': ' 李澈 '},
          <String, Object?>{},
          {'name': '沈微'},
    ...
    final msg = engine.agentMessages(projectId).last;
    expect(msg.role, agentRoleTool);
    expect(msg.toolName, 'custom_script_optional_method_call_runtime');
    expect(jsonDecode(msg.content), {
      'labels': '李澈、未命名、沈微',
      'missing': '默认值',
    });
...
''',
  ...
  test('Agent 记忆：memory_get 和 deepRetrieve 支持 snake_case 查询计划别名', () async {
    final now = DateTime.now().millisecondsSinceEpoch;
    db.execute(
      'INSERT OR REPLACE INTO o_setting (key,value) VALUES (?,?)',
      ['agent.memory.ragLimit', '1'],
    ...
    insertMessage(
      'query_plan_snake_get_keep',
      '角色约束：沈微必须保留镜湖线索，不能删掉。',
      0,
    );
    insertMessage(
      'query_plan_snake_deep_keep',
      '场景约束：镜湖结界需要冷青色反光和碎裂符文。',
      1,
    );
    insertMessage(
      'query_plan_snake_noise',
      '噪声记忆：市集桥段可以加入热闹叫卖声。',
      2,
    );
    ...
    expect(
      memoryGetProperties.keys,
      containsAll(['query_plan', 'retrieval_plan', 'search_plan']),
    );
    final deepRetrieveTool =
        gateway.lastTools.singleWhere((tool) => tool.name == 'deepRetrieve');
    final deepRetrieveProperties = deepRetrieveTool.schema['properties'] as Map;
    ...
    final memoryGetPayload =
        jsonDecode(toolMessages.first.content) as Map<String, dynamic>;
    expect(memoryGetPayload['found'], isTrue);
    expect(memoryGetPayload['queries'], ['沈微镜湖线索']);
    expect(memoryGetPayload['memories'], contains('角色约束：沈微必须保留镜湖线索，不能删掉。'));
    expect(
      memoryGetPayload['memories'],
      isNot(contains('噪声记忆：市集桥段可以加入热闹叫卖声。')),
    );

    final deepRetrievePayload =
        jsonDecode(toolMessages.last.content) as Map<String, dynamic>;
    expect(deepRetrievePayload['found'], isTrue);
    expect(deepRetrievePayload['queries'], ['镜湖冷青色碎裂符文']);
    expect(deepRetrievePayload['memories'], contains('场景约束：镜湖结界需要冷青色反光和碎裂符文。'));
    expect(
      deepRetrievePayload['memories'],
      isNot(contains('噪声记忆：市集桥段可以加入热闹叫卖声。')),
    );
  ...
  test('Agent 记忆：queryPlan filter 支持 must/must_not DSL 别名', () async {
    final now = DateTime.now().millisecondsSinceEpoch;
    db.execute(
      'INSERT OR REPLACE INTO o_setting (key,value) VALUES (?,?)',
      ['agent.memory.ragLimit', '5'],
    ...
    insertMessage(
      'query_plan_dsl_filter_memory_keep',
      '山门风格甲子：首镜必须保持冷白云雾和低机位压迫感。',
      0,
    );
    insertMessage(
      'query_plan_dsl_filter_memory_noise',
      '山门风格甲子：旧版改成暖色喜剧和高机位俯拍。',
      1,
    );
    insertMessage(
      'query_plan_dsl_filter_deep_keep',
      '镜湖动作乙卯：李澈进入水面后必须贴地跟拍，保持肃杀。',
      2,
    );
    insertMessage(
      'query_plan_dsl_filter_deep_noise',
      '镜湖动作乙卯：旧版让李澈进入水面后改成俯拍和暖色。',
      3,
    );
    ...
    for (final tool in [memoryGetTool, deepRetrieveTool]) {
      final properties = tool.schema['properties'] as Map;
      expect(properties, contains('must'));
      expect(properties, contains('must_not'));
      expect(properties, contains('require'));
      expect(properties, contains('not'));
    }
  ...
  test('Agent 记忆：queryPlan filter 支持 bool.must/must_not 包裹', () async {
    final now = DateTime.now().millisecondsSinceEpoch;
    db.execute(
      'INSERT OR REPLACE INTO o_setting (key,value) VALUES (?,?)',
      ['agent.memory.ragLimit', '5'],
    ...
    insertMessage(
      'query_plan_bool_filter_get_keep',
      '石桥调度甲子：第二镜必须保留冷白石桥、低机位推进。',
      0,
    );
    insertMessage(
      'query_plan_bool_filter_get_noise',
      '石桥调度甲子：旧设定把第二镜改成暖色高机位。',
      1,
    );
    insertMessage(
      'query_plan_bool_filter_deep_keep',
      '云台调度乙卯：沈微入画时必须保留蓝火和背光剪影。',
      2,
    );
    insertMessage(
      'query_plan_bool_filter_deep_noise',
      '云台调度乙卯：旧版删除蓝火，改成暖色正面光。',
      3,
    );
    ...
    for (final tool in [memoryGetTool, deepRetrieveTool]) {
      final properties = tool.schema['properties'] as Map;
      expect(properties, contains('bool'));
      expect(properties, contains('布尔条件'));
    }
  ...
  test('Agent 记忆：queryPlan filter 支持 term/match 对象项', () async {
    ...
    insertMessage(
      'query_plan_object_filter_get_keep',
      '石桥调度丙辰：首镜必须保留冷白石桥和低机位推进。',
      0,
    );
    insertMessage(
      'query_plan_object_filter_get_noise',
      '石桥调度丙辰：旧设定改成暖色高机位。',
      1,
    );
    insertMessage(
      'query_plan_object_filter_deep_keep',
      '云台调度丁巳：沈微入画必须保留蓝火背光剪影。',
      2,
    );
    insertMessage(
      'query_plan_object_filter_deep_noise',
      '云台调度丁巳：旧版删除蓝火并改成暖色正面光。',
      3,
    );
    ...
    await engine.sendAgentMessage(
      projectId,
      '按 Elasticsearch 风格对象过滤条件召回制作记忆',
      autoMode: true,
      family: agentFamilyScript,
    );
    ...
    for (final tool in [memoryGetTool, deepRetrieveTool]) {
      final properties = tool.schema['properties'] as Map;
      expect(properties, contains('term'));
      expect(properties, contains('match_phrase'));
      for (final key in const ['must', 'must_not']) {
        final property = properties[key] as Map;
        final items = property['items'] as Map;
        final itemType = items['type'];
        expect(itemType, contains('object'));
      }
    }
  ...
  test('Agent 记忆：queryPlan 支持 dis_max 多查询择优子句', () async {
    final now = DateTime.now().millisecondsSinceEpoch;
    db.execute(
      'INSERT OR REPLACE INTO o_setting (key,value) VALUES (?,?)',
      ['agent.memory.ragLimit', '5'],
    ...
    insertMessage(
      'query_plan_dis_max_get_keep',
      '赤鸢祭台甲：首镜必须保留赤鸢祭台、红色法阵和俯冲转场。',
      0,
    );
    insertMessage(
      'query_plan_dis_max_get_noise',
      '赤鸢祭台甲：只记录普通远景，没有关键视觉动作。',
      1,
    );
    insertMessage(
      'query_plan_dis_max_deep_keep',
      '霜河断桥乙：沈微过桥时必须保留霜河断桥和侧逆光剪影。',
      2,
    );
    insertMessage(
      'query_plan_dis_max_deep_noise',
      '霜河断桥乙：只记录普通远景，没有角色过桥动作。',
      3,
    );
        ...
        'queryPlan': [
          {
            'query': {
              'dis_max': {
                'queries': [
                  {
                    'match_phrase': {'content': '赤鸢祭台 红色法阵'},
                  },
                  {
                    'match': {'content': '俯冲转场'},
                  },
        ...
        'queryPlan': [
          {
            'query': {
              'dis_max': {
                'queries': [
                  {
                    'match_phrase': {'content': '霜河断桥 侧逆光剪影'},
                  },
                  {
                    'query_string': {'query': '沈微 过桥'},
                  },
                ],
              },
            },
    ...
    for (final tool in [memoryGetTool, deepRetrieveTool]) {
      final properties = tool.schema['properties'] as Map;
      expect(properties, contains('dis_max'));
      expect(properties, contains('disMax'));
    }
  ...
  test('Agent 记忆：ES term/terms 字段过滤映射到角色和范围', () async {
    final now = DateTime.now().millisecondsSinceEpoch;
    db.execute(
      'INSERT OR REPLACE INTO o_setting (key,value) VALUES (?,?)',
      ['agent.memory.ragLimit', '5'],
    ...
    insertMessage(
      id: 'query_plan_term_role_user_noise',
      content: '霜燃灯令甲：保持低机位压迫感。',
      offset: 0,
      role: agentRoleUser,
    );
    insertMessage(
      id: 'query_plan_term_role_execution',
      content: '霜燃灯令甲：保持低机位压迫感。',
      offset: 1,
      role: 'assistant:execution:script',
    );
    insertMessage(
      id: 'query_plan_terms_scope_message_noise',
      content: '星砂罗盘乙：普通对话里说罗盘可改成暖色。',
      offset: 2,
      role: agentRoleUser,
    );
    final noteId = engine.saveAgentMemory(
      projectId,
      name: '星砂罗盘长期设定',
      content: '星砂罗盘乙：长期设定必须保持冷蓝反光和环形推轨。',
    );
    ...
    expect(
      (memoryGetPayload['records'] as List)
          .map((record) => (record as Map<String, dynamic>)['id']),
      ['query_plan_term_role_execution'],
    );
    expect(
      (memoryGetPayload['records'] as List)
          .map((record) => (record as Map<String, dynamic>)['role']),
  ...
  test('Agent 记忆：queryPlan 支持 ES match_none 空结果查询', () async {
    final now = DateTime.now().millisecondsSinceEpoch;
    db.execute(
      'INSERT OR REPLACE INTO o_setting (key,value) VALUES (?,?)',
      ['agent.memory.ragLimit', '5'],
    ...
    db.execute(
      ...
      'VALUES (?,?,?,?,?,?,?,?,?,?)',
      [
        'query_plan_match_none_noise',
        '',
        'match_none 噪声记忆：这条不应被返回。',
        now,
        embeddingJson('match_none 噪声记忆：这条不应被返回。'),
        'scriptAgent:$projectId',
        '[]',
        agentRoleAssistant,
        1,
    ...
    for (final tool in [memoryGetTool, deepRetrieveTool]) {
      final properties = tool.schema['properties'] as Map;
      expect(properties, contains('match_none'));
      expect(properties, contains('matchNone'));
    }
    ...
    for (final message in toolMessages) {
      final payload = jsonDecode(message.content) as Map<String, dynamic>;
      expect(payload['found'], isFalse);
      expect(payload['queries'], ['match_none']);
      expect(payload['records'], isEmpty);
      expect(
          jsonEncode(payload), isNot(contains('query_plan_match_none_noise')));
    }
  });
  ...
  test('Agent 记忆：memory_update 和 memory_delete 可从 record 对象抽取长期记忆 id',
      () async {
    final updateId = engine.saveAgentMemory(
      projectId,
      id: 'tool_record_update_long_term_note',
      name: '旧镜湖约束',
      content: '长期设定：镜湖线索可以删掉。',
    );
    final deleteId = engine.saveAgentMemory(
      projectId,
      id: 'tool_record_delete_long_term_note',
      name: '过期设定',
      content: '长期设定：沈微已离开主线。',
    );
    final keepId = engine.saveAgentMemory(
      projectId,
      id: 'tool_record_keep_long_term_note',
      name: '保留设定',
      content: '长期设定：沈微必须保留镜湖线索。',
    );
    gateway.turns = [
      AgentTurnResult.tool('memory_update', {
        'record': {
    ...
    final updateTool =
        gateway.lastTools.singleWhere((tool) => tool.name == 'memory_update');
    final updateProperties = updateTool.schema['properties'] as Map;
    expect(updateProperties.keys, containsAll(['record', 'records']));
    final deleteTool =
        gateway.lastTools.singleWhere((tool) => tool.name == 'memory_delete');
    final deleteProperties = deleteTool.schema['properties'] as Map;
    expect(deleteProperties.keys, containsAll(['record', 'records']));
    ...
    final updatePayload =
        jsonDecode(toolMessages.first.content) as Map<String, dynamic>;
    expect(updatePayload['updated'], isTrue);
    expect(updatePayload['id'], updateId);
    expect(updatePayload['content'], '长期设定：镜湖线索必须保留，不能删掉。');

    final deletePayload =
        jsonDecode(toolMessages.last.content) as Map<String, dynamic>;
    expect(deletePayload['deleted'], isTrue);
    expect(deletePayload['id'], deleteId);

    final longTerm = engine.agentLongTermMemories(projectId);
    expect(
        longTerm.map((memory) => memory.id), containsAll([updateId, keepId]));
    expect(longTerm.map((memory) => memory.id), isNot(contains(deleteId)));
    expect(longTerm.singleWhere((memory) => memory.id == updateId).content,
        '长期设定：镜湖线索必须保留，不能删掉。');
  });
  ...
  test('Agent 记忆：queryPlan range 支持 ISO 时间字符串', () async {
    final baseTime = DateTime.utc(2026, 1, 1, 12).millisecondsSinceEpoch;
    void insertMemory({
      required String id,
      required String isolationKey,
      required String content,
    ...
    insertMemory(
      id: 'range_iso_old_msg',
      isolationKey: 'scriptAgent:$projectId',
      content: 'ISO窗口记忆：星桥校准在旧阶段记录旧路线。',
      createTime: baseTime,
      type: agentMemoryTypeMessage,
      summarized: 1,
    );
    insertMemory(
      id: 'range_iso_keep_msg',
      isolationKey: 'scriptAgent:$projectId',
      content: 'ISO窗口记忆：星桥校准在窗口内记录冷光符路线。',
      createTime: baseTime + 1000,
      type: agentMemoryTypeMessage,
      summarized: 1,
    );
    insertMemory(
      id: 'range_iso_new_msg',
      isolationKey: 'scriptAgent:$projectId',
      content: 'ISO窗口记忆：星桥校准在新阶段记录南门路线。',
      createTime: baseTime + 2000,
      type: agentMemoryTypeMessage,
      summarized: 1,
    );
    insertMemory(
      id: 'range_iso_old_note',
      isolationKey: 'project:$projectId',
      content: 'ISO窗口长期记忆：镜湖航线旧版本经过旧渡口。',
      createTime: baseTime,
      type: agentMemoryTypeNote,
      name: '旧镜湖航线',
    );
    insertMemory(
      id: 'range_iso_keep_note',
      isolationKey: 'project:$projectId',
      content: 'ISO窗口长期记忆：镜湖航线窗口内经过灯塔。',
      createTime: baseTime + 1000,
      type: agentMemoryTypeNote,
      name: '灯塔镜湖航线',
    );
    insertMemory(
      id: 'range_iso_new_note',
      isolationKey: 'project:$projectId',
      content: 'ISO窗口长期记忆：镜湖航线新版本经过北桥。',
      createTime: baseTime + 2000,
      type: agentMemoryTypeNote,
      name: '北桥镜湖航线',
    );
    ...
    gateway.turns = [
      AgentTurnResult.tool('memory_get', {
        'queryPlan': [
          {
            'query': {
              'match_all': {},
            ...
            'range': {
              'createTime': {
                'gte': DateTime.fromMillisecondsSinceEpoch(
                  baseTime + 500,
                  isUtc: true,
                ).toIso8601String(),
                'lte': DateTime.fromMillisecondsSinceEpoch(
                  baseTime + 1500,
                  isUtc: true,
                ).toIso8601String(),
              },
  ...
  test('Agent 记忆：queryPlan range 支持 Elasticsearch now 时间表达式', () async {
    final now = DateTime.now().millisecondsSinceEpoch;
    void insertMemory({
      required String id,
      required String isolationKey,
    ...
    insertMemory(
      id: 'range_now_old_msg',
      isolationKey: 'scriptAgent:$projectId',
      content: 'now窗口记忆：云舟校验旧阶段保留旧口令。',
      createTime: now - const Duration(minutes: 10).inMilliseconds,
      type: agentMemoryTypeMessage,
      summarized: 1,
    );
    insertMemory(
      id: 'range_now_keep_msg',
      isolationKey: 'scriptAgent:$projectId',
      content: 'now窗口记忆：云舟校验当前阶段保留新口令。',
      createTime: now,
      type: agentMemoryTypeMessage,
      summarized: 1,
    );
    insertMemory(
      id: 'range_now_future_msg',
      isolationKey: 'scriptAgent:$projectId',
      content: 'now窗口记忆：云舟校验未来阶段保留预案口令。',
      createTime: now + const Duration(minutes: 10).inMilliseconds,
      type: agentMemoryTypeMessage,
      summarized: 1,
    );
    insertMemory(
      id: 'range_now_old_note',
      isolationKey: 'project:$projectId',
      content: 'now窗口长期记忆：云舟航线旧阶段走西门。',
      createTime: now - const Duration(minutes: 10).inMilliseconds,
      type: agentMemoryTypeNote,
      name: '旧云舟航线',
    );
    insertMemory(
      id: 'range_now_keep_note',
      isolationKey: 'project:$projectId',
      content: 'now窗口长期记忆：云舟航线当前阶段走东门。',
      createTime: now,
      type: agentMemoryTypeNote,
      name: '当前云舟航线',
    );
    insertMemory(
      id: 'range_now_future_note',
      isolationKey: 'project:$projectId',
      content: 'now窗口长期记忆：云舟航线未来阶段走北门。',
      createTime: now + const Duration(minutes: 10).inMilliseconds,
      type: agentMemoryTypeNote,
      name: '未来云舟航线',
    );
