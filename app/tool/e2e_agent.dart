// P5 E2E：Agent 多工具自由选择真实调用（需 azt @8787）。
...
Future<void> main() async {
  ...
  try {
    ...
    engine.addScript(projectId: projectId, name: '雪夜', content: '林朝雪拔剑而立，望向雪夜中的来客。');

    stdout.writeln('[p5] 询问进度…');
    await engine.sendAgentMessage(projectId, '现在项目进度如何？', autoMode: false);
    for (final m in engine.agentMessages(projectId)) {
      stdout.writeln('   [${m.role}${m.toolName != null ? ":${m.toolName}" : ""}] '
          '${m.content.substring(0, m.content.length > 80 ? 80 : m.content.length)}');
    }
    final afterStatus = engine.agentMessages(projectId);
    if (afterStatus.length < 2) throw StateError('未收到助手回复');

    stdout.writeln('[p5] 请求提取资产…');
    await engine.sendAgentMessage(projectId, '帮我把这个剧本的角色道具场景提取一下', autoMode: false);
    for (final m in engine.agentMessages(projectId).skip(afterStatus.length)) {
      stdout.writeln('   [${m.role}${m.toolName != null ? ":${m.toolName}" : ""}] '
          '${m.content.substring(0, m.content.length > 80 ? 80 : m.content.length)}');
    }
    final tasks = await engine.projectJobs(projectId);
    if (!tasks.any((t) => t.taskClass == 'asset_extraction')) {
      throw StateError('Agent 未能正确调用 extract_assets 工具（任务表无 asset_extraction 记录）');
    }
