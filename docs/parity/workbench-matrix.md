...
| --- | --- | --- | --- |
| 打开工作台 | 全屏三页签：快速预览、视频生成、视频剪辑；默认预览；按 `videoRatio` 初始化剪辑画布 | 同一全屏页顶部三个图标 Tab，默认内嵌快速预览；预览舞台按项目 `16:9`/`1:1`/`9:16` 映射，生成和剪辑均可切换 | 部分实现：三工作面、默认入口与预览画幅已验证；生成面保留时间线快捷概览，剪辑面尚无实时画布，结构仍非严格等价 |
...
- 剪辑工作面：接入可真实预览时间线层的画布与可控导出路径；不能用静态比例框冒充
  `editVideo` 的实时画布。
- 生成工作面：在不丢失时间线测试覆盖的前提下，将快捷时间线概览迁回剪辑工作面，消除
  与原版 generate Tab 的多余可见内容。
