...
| --- | --- | --- |
| `app/lib/src/screens/script/script_screen.dart:110-146,219,373-442` | 工具栏提供“从事件生成剧本”，弹出 `_EventScriptPicker`，读取 `Engine.events` 并提交任务 | **Flutter 独有可达入口。** |
...
`W6-SCRIPT-001` 不能继续标为“已验证等价”：核心 CRUD、导入、批量添加、导出和资产提取已有回归，但“从事件生成剧本”是额外可达动作。该项先降为**部分实现**。

下一步应先区分两层：

1. 移除 `/script` 页面上的事件生成入口和 `_EventScriptPicker`，使手动剧本管理页与原版一致。
2. 再审计 Agent 的“从章节事件生成剧本”是否应依原版 `scriptAgent` 行为重建，还是作为整个 Agent 模块的缺失项处理；不得用现有 `o_event` 简化链替代原版 Agent。

在这两项完成前，`o_event/o_eventChapter` 的额外写入继续由 `W9A-DBTABLE-EVENT-001` 跟踪，不能上调为等价。
