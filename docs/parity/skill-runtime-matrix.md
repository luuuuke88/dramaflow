...
| --- | --- | --- | --- |
| 技能文件库管理 | 树形浏览、搜索、预览、编辑并保存既有 `.md` 文件 | 应用自有工作区的相对路径目录树、搜索、预览、编辑保存和启停；桌面双栏、窄屏逐层进入/返回 | 已验证等价 |
...
Flutter 现在通过 `_AssistantSkillsPane` 提供应用自有 `dataDir/skills/<id>/` 的相对路径
目录树：只列常规 `.md`，桌面可递归展开，窄屏可逐层进入并在文件详情返回时逐层退出；搜索
直接列完整相对路径。任一既有 Markdown 都可预览和编辑；`SKILL.md` 保存继续同步稳定 ID、
名称和描述，深层资源只替换文件本身。外部绝对路径不会写入数据库；导入 `SKILL.md` 时会复制
同目录常规资源，因此源文件和资源删除后，托管内容仍可读取。`assistant_skill_library_test.dart`、
`assistant_skills_deploy_test.dart` 与 `agent_chat_screen_test.dart` 覆盖树排序、深层编辑、
符号链接/路径逃逸拒绝、1200dp 展开与 390dp 进入、返回和搜索。

`W6D-SKILL-001` 已验证等价。目录导入、ZIP、任意外部目录浏览、创建/删除/重命名不是原版
此页的既有用户动作，也没有随本项加入；它们不构成该行缺口。
...
- 文件管理：已验证相对路径目录树、深层 Markdown 预览/编辑保存、搜索、1200dp 展开和
  390dp 逐层进入/返回；目录导入、ZIP、任意外部目录、创建/删除/重命名不属于原版此页范围。
