...
| 目的 | 文档 | 说明 |
| --- | --- | --- |
| 看所有功能的权威结论 | [总对照清单](master-checklist.md) | 202 条功能聚合行，逐条链接 ToonFlow 与 DramaFlow 源码、数据依赖、测试和缺口。 |
...
总清单把多个源码文件按用户旅程聚合为 202 条结论，当前分布如下：

| 状态 | 条数 | 含义 |
| --- | ---: | --- |
| 已验证等价 | 70 | 已有用户行为、源码和针对性的自动化证据。 |
| 部分实现 | 98 | 核心或部分路径存在，但仍有明确可观察缺口。 |
| 缺失 | 20 | ToonFlow 的有效用户能力没有对应实现。 |
| 不适用 | 13 | Electron/浏览器架构特有，且原生 Flutter 形态没有对应用户价值。 |
...
Agent 体系、NLE 素材/预览能力，以及原版供应商私有协议适配。项目创建的完整准入保护已由
`W6-PROJECT-001` 验证收口，不再列作待办。每项的具体范围、而不是泛泛的“待优化”，已经在
总清单和对应专项文档中落了源码证据。

### 最近收口：制作画布首次操作引导

`W6E-CMP-PRODUCTION-GUIDE-001` 已从缺失转为已验证等价。原版以
`productionCurrent` 保存的四步教学，在 Flutter 中以设备级 SQLite 键
`production.guide.completed` 实现：桌面端高亮剧集、刷新、自动布局和画布四个真实控件；
390dp 以可滚动的全屏步骤页替代不可用的悬浮卡。完成或跳过后不再重复出现；窗口在紧凑/桌面阈值间变化时会重新测量真实控件，避免遗留旧高亮坐标。

同一批次还补齐了原版自动布局的视口回正：恢复节点排布后调用 `fitView()`，并修复缩小
视口时节点拖动按错误倍率换算的问题。刷新会重建当前本地画布，使节点位置、视口和聊天面板回到初始状态并重新 `fitView()`。证据为 `production_guide_test.dart`、
`production_screen_test.dart`、`df_widgets_test.dart` 和 `config_test.dart`；全部使用内存数据库、
本地夹具与 fake gateway，不调用任何视频或其他供应商。

### 最近收口：画布空格平移

原版 `Space + 左键` 现在由共享 `DFCanvas` 对齐，而不是只在制作页特判。背景手势层
负责平移、缩放、滚轮与触控板；Space 专用识别器只在鼠标主键组合时抢占卡片内部手势。
因此节点标题栏、图片流卡片和图片组件内的手势都不会误改节点，普通触摸、文本输入和参数
选择也不会被画布抢走。桌面鼠标拖拽/滚轮、触控板、移动端双指、2 倍和 0.5 倍 Space
平移、桌面/390dp 节点位置持久化及参数区隔离都有 widget 回归证据，全程不调用供应商。
