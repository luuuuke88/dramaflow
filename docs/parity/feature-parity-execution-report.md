...
另有 1 条 W2 前置证据行仅记录“旧 DramaFlow 自行膨胀出的 JS DSL/解释器并非 ToonFlow 原能力”，它不是用户功能，故不参与六态统计。已验证等价与不适用合计 **84/201（41.8%）**；以“用户可用的完整复刻”为标准，项目仍处于中段，不能宣称接近 100%。
...
5. **消化设置与长尾缺口**：处理第 4 节 21 项和主清单余下的部分实现，逐项关闭。
