...
当前 Flutter 版已经能编辑既有 `o_modelPrompt` 行，但尚未提供 ToonFlow 的“可复用模板库 -> 选择绑定到模型 -> 解绑”闭环。该缺口会直接放大运行目录与模型模板不一致的风险，尤其影响 Seedance 的多参模板选择。本轮将用本地 SQLite 模板库替代 Electron 的运行目录 Markdown 文件，并保持映射解析、旧配置导入和既有模型正文编辑兼容；验收只使用内存数据库和假网关，不发起任何视频或其他供应商生成。
