...
另有 1 条 W2 前置证据行仅记录“旧 DramaFlow 自行膨胀出的 JS DSL/解释器并非 ToonFlow 原能力”，它不是用户功能，故不参与六态统计。已验证等价与不适用合计 **83/201（41.3%）**；以“用户可用的完整复刻”为标准，项目仍处于中段，不能宣称接近 100%。
...
| 组别 | 主清单 ID | 缺口 |
| --- | --- | --- |
| 制作体验 | `W6B-NODE-POSTER-001` | 制作画布中的海报/封面节点 |
| 首启与技能 | `W6E-CMP-HELLO-001`、`W6E-LIB-SCANSKILLS-001` | 首次启动引导、技能目录重扫 |
...
制作画布的节点骨架和编辑入口已有，但无限画布手感、缩放围绕光标、拖拽、触控板、选区、自动布局以及大量节点时的帧率尚未达到可验收状态。参考资料为 [`w1-canvas-reference.md`](w1-canvas-reference.md)。

完成定义：同一套画布在 macOS 鼠标/触控板与 iOS/Android 手势下都能完成平移、缩放、节点编辑；性能指标必须通过 profile 实测，而不是凭视觉判断。
...
供应商预设、模型选择、画风库、导入导出、设置高级项和移动版对话框目前正在开发，工作区仍有未提交改动。移动发现记录位于 [`mobile-adaptation-findings.md`](mobile-adaptation-findings.md)。这些改动在合并前不得因“桌面能用”而标绿。
...
5. **消化设置与长尾缺口**：处理第 4 节 22 项和主清单余下的部分实现，逐项关闭。
