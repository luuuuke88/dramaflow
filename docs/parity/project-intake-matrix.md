...
项目创建、编辑与打开的用户旅程已闭合，但删除链路仍有一项 Flutter 自身扩展带来的数据卫生缺口：
`events.dart` 会把章节事件额外写入 `o_event` 与 `o_eventChapter`，而
`Engine.deleteProject()` 当前直接删除 `o_novel`，未一并清理这两张关联表。页面读取都经
`o_novel` 内连接，因此孤儿行暂时不会显示给用户；它仍须在 W4 长尾清零阶段改为复用
`deleteNovels()` 的清理语义，并补一条删除项目后的两表归零回归。这个内部缺口不改变本矩阵对
项目创建与打开旅程的等价结论，也不应被悄悄掩盖。
