...
结果为 **126 项通过**。视频相关用例只使用 Dart fake gateway 或本地假合成器，
...
| 属性面板 | 常驻停靠面板：名称/起止/总时长/不透明度/音量/播放倍速/音频淡入淡出/转场类型+时长/字幕文本+字号，复制/删除 | 桌面选中叠加层后显示固定 `_TimelineClipInspector`，可编辑名称、轨道、起始、时长和不透明度；菜单编辑与移动端仍使用 `_EditTimelineClipDialog`/全屏表单。`o_timelineClip` 没有音量、倍速、淡入淡出、字幕或转场字段，故剩余属性不是“补几个控件”即可。 |
...
1. **已完成，常驻属性检查器**：桌面端勾选叠加层后显示固定检查器，编辑名称、轨道、起始、时长和不透明度；菜单编辑仍作为完整表单保留，窄屏继续以全屏表单呈现。检查器不注册拖拽手势，避免破坏时间线吸附、裁剪和移动。
2. **已完成，现有名称的编辑入口**：`updateTimelineClip(name:)` 受本地 SQLite 回归保护，桌面检查器和移动/菜单表单共用同一写入路径。它只解决已有 `name` 列的可访问性，不把当前叠加层包装成完整多媒体属性模型。
...
DramaFlow 的"叠加层时间线"有合理且独到之处（波纹/分组/吸附/避让是 ToonFlow 没有的能力），但它不是原版自由多轨 NLE 的底座。W3 的首要事实是：`o_timelineClip` 不具备原版片段属性的数据模型，不能把缺口包装成“解冻”。2026-07-21 已关闭已有字段的桌面检查器和名称编辑入口；真正剩余的工作仍是确立自由素材片段模型，再补转场/滤镜/特效、实时预览、撤销重做和平台合成覆盖。
