...
音频父资产展开全部样本、绑定音频自动追加和引用持久化，且已有桌面/390dp widget 与引擎
回归。它仍与原版有一个可观察差异：原版在“取消资产选择”后直接转入分镜网格，而 Flutter
先显示明确的来源选择。因此本行仍为“部分实现”，不能因核心素材集合已经可达就报成完整复刻。
...
- `image_flow_editor_test.dart` 覆盖三来源菜单、从资产/分镜选图、分镜空态，以及
  390dp 下选资产后保存参考图；
- `workbench_screen_test.dart` 覆盖多参考、纯文生隐藏参考、首尾帧角色、390dp 参数保存、完整素材库选择的媒体类型过滤、同剧集分镜选择、音频父资产展开全部子样本，以及选择项目角色时自动携带绑定音频；
...
稳定 ID；视频请求仍负责按模型能力对已选引用做最后校验。剩余差异只在原版“取消即切换
分镜”与 Flutter 显式来源选择的入口时序，不应再为此复制另一套选择器。
