...
资产来源使用自适应网格，点击即返回一张本地相对路径；分镜来源改由
...
它仍未实现原版的 `clip` 图片/视频/音频子类型过滤；图片流与视频参考面板也还没有统一
接入此入口。因此它是可复用的基础选择器，但不是原版所有调用点都已迁移完毕的结论。
...
显示）、**项目全部可用的角色/道具/场景/片段/音频**，以及项目中已完成的视频。选择一个
绑定了音色的非音频资产时，弹窗会在音频参考上限内自动加入其绑定音频。

这覆盖模式、首尾帧角色、多参考限额、项目级素材发现、绑定音频自动追加和引用持久化，
且已有桌面/390dp widget 与引擎回归。它仍不是原版完整资产选择器：没有搜索、分页、按
片段真实媒体类型筛选，也没有“取消资产选择后转选分镜”的独立交互。因此这行仍为“部分
实现”，不能因核心素材集合已经可达就报成完整复刻。
...
| 图片/视频/音频数量限制 | 有 | 有 | 等价 |
| 从完整项目资产库挑选 | 有 | 有（现有视频参数弹窗中的项目素材分组） | 核心结果等价 |
...
- `image_flow_editor_test.dart` 覆盖三来源菜单、从资产/分镜选图、分镜空态，以及
  390dp 下选资产后保存参考图；
- `workbench_screen_test.dart` 覆盖多参考、纯文生隐藏参考、首尾帧角色、390dp 参数保存，以及选择项目角色时自动携带绑定音频；
- `video_track_test.dart` 验证候选只接受本地存在媒体，并覆盖项目级角色/片段/音频素材进入候选；
