...
种子从同一目录构建，防漂移测试锁定两者逐字段一致。启动时还会识别完全
未编辑的 2026-07-18 旧默认目录并升级；只要用户新增模型、改标签、关闭
模型或改动本机端点，指纹即不匹配，原配置不会被覆盖。`/models` 不返回
`gpt-image-2` 不能单独推断图片不可用，图片能力继续以独立模态证据判断。
这只关闭“静态文本模型过期”的缺口，不改变 azt 在图片参数遵从、多参考和
移动端可达性上的其余部分实现状态。
...
3. 不以 13 张新供应商画廊预设替代原版 13 个文件的逐项复刻。国际新预设可保留，但必须作为扩展项，和原版基线行分开计分。
