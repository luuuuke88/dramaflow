...
Known Gaps: Full WebAV/NLE parity is not implemented: Android dissolve/whip-pan rendering, Web transition/filter visual rendering, Web cross-shot dissolve, Apple combined dissolve plus CoreImage filter rendering, Apple/Android/Web whip-pan rendering, multi-layer timeline editing, and drag-based clip editing are not proven.
