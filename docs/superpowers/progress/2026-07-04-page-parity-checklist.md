...
Known Gaps: Full WebAV/NLE parity is not implemented: Web transition/filter visual rendering, Web cross-shot dissolve, Web whip-pan rendering, complex overlap/ripple editing, complex snapping rules, and full drag-and-drop media-bin/freeform timeline editing are not proven. Current media-library support can attach existing clip assets to a shot as selected candidates, save generated candidates back as clip assets, add clip assets as overlay-layer records, drag-adjust overlay lane/start positions, show a basic snapping guide while dragging near a timeline anchor, edge-trim overlay start/duration, avoid basic same-lane overlap during drag/edge-trim, midpoint-split overlay clips, playhead-split overlay clips, remove overlay clips from the timeline without deleting source assets, and ripple-delete overlay clips; it is still not a full ToonFlow WebAV clip editor.
