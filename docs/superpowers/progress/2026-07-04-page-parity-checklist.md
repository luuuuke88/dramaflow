...
Known Gaps: Full WebAV/NLE parity is not implemented: Web transition/filter visual rendering, Web cross-shot dissolve, Web whip-pan rendering, multi-layer timeline editing, and drag-based multi-layer clip editing are not proven. Current media-library support can attach existing clip assets to a shot as selected candidates; it is not a full drag-and-drop media bin or freeform timeline.
