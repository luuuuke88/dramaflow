...
Known Gaps: Full WebAV/NLE parity is not implemented: Web transition/filter visual rendering, Web cross-shot dissolve, Web whip-pan rendering, advanced overlap/ripple editing, visual snapping guides/complex snapping rules, and full drag-and-drop media-bin/freeform timeline editing are not proven. Current media-library support can attach existing clip assets to a shot as selected candidates, save generated candidates back as clip assets, add clip assets as overlay-layer records, drag-adjust overlay lane/start positions, edge-trim overlay start/duration, midpoint-split overlay clips, playhead-split overlay clips, and remove overlay clips from the timeline without deleting source assets; it is still not a full ToonFlow WebAV clip editor.
