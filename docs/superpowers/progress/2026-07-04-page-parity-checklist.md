...
Known Gaps: Full WebAV/NLE parity is not implemented: Web transition/filter visual rendering, Web cross-shot dissolve, Web whip-pan rendering, overlay clip trimming/resizing, and full drag-and-drop media-bin/freeform timeline editing are not proven. Current media-library support can attach existing clip assets to a shot as selected candidates, save generated candidates back as clip assets, add clip assets as overlay-layer records, and drag-adjust overlay lane/start positions; it is still not a full ToonFlow WebAV clip editor.
