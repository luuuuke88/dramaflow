...
Current v0.4 Correction: Agent 页现在是 `assistant_*` 降级版，而不是下面历史记录里的旧 `agent.dart` 扩张版。

- ES-DSL / queryPlan 解释器已于 v0.4 移除；不要再把 Elasticsearch-style 检索兼容项当作当前能力。
- 自定义 JS 技能执行 / `custom-js-agent` 运行时已于 v0.4 移除；当前只保留 Markdown 技能 CRUD、开关和提示词上下文注入。
- 多层 decision/execution/supervision 监督编排已于 v0.4 移除；当前为 script / production 两个入口家族的单层对话式 assistant。
- 向量 RAG / `o_memoryVector` 读写路径已于 v0.4 移除；schema 表仍在，运行时不再写入或读取。

Current Desktop Evidence: `app/test/engine/assistant_actions_test.dart`, `app/test/engine/assistant_chat_test.dart`, `app/test/engine/assistant_skills_deploy_test.dart`, `app/test/engine/project_notes_test.dart`, `app/test/widgets/agent_chat_screen_test.dart`, and `app/test/widgets/canvas_chat_panel_test.dart` cover the current assistant action registry, argument normalization, manual/auto chat loop, confirmation suspension, Markdown skills/deployments, project notes search, script/production entry routing, and UI removal of custom JS / supervision / RAG settings.

Historical Pre-v0.4 Evidence: The long Agent/RAG notes below are retained only to explain what was tried before v0.4 convergence. They are superseded by the current correction above and must not be used as a restoration checklist.
...
Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers ScriptAgent `get_planData` accepting ToonFlow/model-authored workspace section aliases such as `故事骨架`, `改编策略`, and `剧本内容`, so script execution sub Agents can read planning workspace segments even when the model emits Chinese semantic keys instead of exact camelCase identifiers.
...
Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` and `deepRetrieve` returning empty `memories` / `records` arrays on no-hit results, with `memory_get` also returning empty `summaries`, `recent`, and `notes`, so downstream Agent tools can parse miss payloads with the same fields used by successful recall.
...
Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` top-level `limit` applying to summary-only retrieval, so Agent decision/supervision prompts can cap historical summaries the same way they cap ordinary memories and notes.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` top-level `limit` applying to recent unsummarized conversation results, so Agent decision/supervision prompts can keep short-term chat context bounded instead of inheriting the full `shortTermLimit` window.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` recent unsummarized conversation results honoring per-item structured query-plan `limit` and `orderBy`, so multi-step Agent plans can retrieve bounded short-term context for each subtask without cross-item bleed.
...
Recent Custom Skill Update: `app/test/engine/agent_test.dart` now covers nested object/array patterns inside array destructuring, so model-authored helpers can unpack cover images, role references, scene tuples, and storyboard rows from ToonFlow-style ordered payloads without rewriting them into manual index access.
...
Recent Custom Skill Update: `app/test/engine/agent_test.dart` now covers custom helper functions invoked through `helper.call(null, ...)` and `helper.apply(null, args)`, so model-authored skills can keep common Function.prototype forwarding style while reusing local `function` declarations and assigned arrow helpers.

Recent Custom Skill Update: `app/test/engine/agent_test.dart` now covers custom helper functions invoked through `helper.bind(null, prefix)` and reused as direct functions or array callbacks, so model-authored skills can pre-bind common labels/context without rewriting helper signatures.
...
Recent Custom Skill Update: `app/test/engine/agent_test.dart` now covers `Promise.allSettled(...).then(...)` chains over mixed fulfilled/rejected records, so model-authored batch scripts can keep normal all-settled post-processing callbacks while preserving readable `reason.message` details.
...
Recent Custom Skill Update: `app/test/engine/agent_test.dart` now covers `Promise.any(...).catch(err => err.errors)` all-rejected flows with an `AggregateError`-style object, so model-authored custom skills can summarize every failed reference/storyboard candidate instead of losing the per-candidate reasons.
...
Recent Custom Skill Update: `app/test/engine/agent_test.dart` now covers sync-compatible `Promise.finally(() => ...)` cleanup chains for both fulfilled and rejected promises, including zero-argument arrow callbacks, so model-authored custom skills can keep cleanup/trace steps while preserving the original Promise state.
...
Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` query plans where a broad low-priority query and a later exact high-priority query hit the same memory, so quick context retrieval merges duplicate hits to the highest priority before global limits just like `deepRetrieve`.
...
Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` and `deepRetrieve` queryPlan items carrying content-level `mustInclude` / `excludeTerms` filters plus Chinese aliases, so decision and sub Agents can require hard context terms while excluding stale or forbidden memory records from the same broad query.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` and `deepRetrieve` records carrying `matchedQuery`, `queryPlanIndex`, and `queryReason` from the highest-priority queryPlan hit, so downstream sub Agents can audit why each memory was recalled instead of treating RAG context as an opaque blob.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` and `deepRetrieve` records carrying `retrievalSource`, `embeddingProvider`, `embeddingModel`, and `embeddingDimension` when an `o_memoryVector` gateway index supplies the hit, so downstream sub Agents can audit whether a memory came from the vector index rather than opaque token matching.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers queryPlan retrieval-source filters such as `retrievalSource: "vector_index"` and `只用向量索引: true`, so downstream sub Agents can require indexed-vector hits and avoid treating local/token fallback matches as equivalent to vector-index recall.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers same-item queryPlan conjunctions such as `match: "all"` and `必须全部命中: true`, so `memory_get` and `deepRetrieve` can require multiple hard constraints to hit the same memory instead of merging separate broad recall results.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers queryPlan fallback items such as `fallback: true` and `仅在无结果时使用: true`, so strict RAG plans can try hard constraints first and only run broader recall when earlier plan items return no memory.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers grouped queryPlan fallback via `group`, `fallbackGroup`, and `检索分组`, so role, scene, and style retrieval lanes can decide fallback independently while still skipping same-group broad recall after a strict hit.
...
Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers per-query-plan model reranking for `deepRetrieve` via the same `rerank` / `模型重排` fields, so deep recall can filter keyword-similar but semantically noisy source messages without forcing global reranking onto every Agent memory lookup.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers per-query-plan model reranking for long-term `note` memories through `memory_get`, and `app/lib/src/engine/agent_memory.dart` applies the same candidate-window rerank path used for dialogue memories so long-term setting recall can reject high-token-noise notes while preserving `relevanceReason`.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Agent system prompts injecting long-term `<note>` blocks with model-rerank `relevanceReason` metadata, so decision, supervision, and sub Agents can see why a project-level setting memory was selected instead of receiving opaque long-term text.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers ScriptAgent and ProductionAgent auto mode pausing after a successful supervision report, so ToonFlow-style supervision works as a user-confirmed quality gate instead of immediately dispatching the next sub-agent.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers ScriptAgent `get_planData` and ProductionAgent `get_flowData` accepting full-workspace aliases such as `all`, `workspace`, `全部`, and `工作区`, so model-authored sub-agents can request the whole workspace without accidentally reading a nonexistent field and getting `无数据`.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_get` and `deepRetrieve` accepting snake_case query-plan aliases such as `query_plan`, `retrieval_plan`, and `search_plan`, so OpenAI-style model-authored RAG plans keep working even when they do not emit ToonFlow's camelCase field names.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers `memory_update` and `memory_delete` accepting `record` / `records` objects returned by `memory_get` and `deepRetrieve`, so multi-step Agents can pass recalled long-term memory records directly into correction/deletion tools without manually extracting ids.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers queryPlan filter/where blocks using DSL-style content filters such as `must`, `must_not`, `require`, and `not`, so model-authored RAG plans can express hard include/exclude memory constraints without rewriting them to DramaFlow-specific `mustInclude` / `excludeTerms` field names.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers queryPlan filter/where blocks wrapping DSL content filters under `bool`, `boolFilter`, `bool_filter`, and `布尔条件`, so model-authored Elasticsearch-style RAG plans such as `filter: { bool: { must, must_not } }` still apply hard include/exclude constraints before memories reach downstream sub-agents.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers queryPlan filter/where blocks carrying Elasticsearch-style object items such as `term`, `match`, and `match_phrase`, so model-authored RAG plans can pass field-wrapped include/exclude phrases without turning them into unusable map-string keywords.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `bool.filter` clauses as hard include constraints for both `memory_get` and `deepRetrieve`, so model-authored plans can use `filter: [{term...}, {match...}]` without accidentally returning broad same-query memories that miss required visual/story constraints.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `bool.should` plus `minimum_should_match` / `minimumShouldMatch`, so model-authored RAG plans can express candidate visual/story constraints where at least one option must match instead of returning broad same-topic memories.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `query`, `constant_score`, and `nested` wrappers around bool filters, so model-authored RAG plans that omit a plain query string can still derive a retrieval query from hard/optional content constraints and apply the wrapped filters before returning memories.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `multi_match`, `query_string`, and `simple_query_string` query clauses, so model-authored RAG plans using common multi-field/search-string DSL can generate concrete `memory_get` / `deepRetrieve` requests instead of collapsing into empty recall.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `dis_max` / `disMax` multi-query clauses, so model-authored RAG plans can pass best-field query wrappers and still expand them into concrete `memory_get` / `deepRetrieve` recall requests.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `function_score` / `functionScore` query wrappers, so model-authored RAG plans can keep score-function envelopes while DramaFlow expands the inner query/filter into concrete `memory_get` / `deepRetrieve` recall requests.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `boosting` positive/negative query wrappers, including direct `match` positive clauses, so model-authored RAG plans can treat positive clauses as required recall context and negative clauses as hard exclusions for stale, old-version, or forbidden memory records.

Recent Agent/RAG Update: `app/test/engine/agent_test.dart` now covers Elasticsearch-style `range.createTime.gte/lte` time-window filters inside query plans, so model-authored RAG plans can constrain `memory_get` / `deepRetrieve` by ToonFlow-style historical windows even when the model emits standard ES range syntax.

Current Mobile Evidence: `app/test/widgets/production_screen_test.dart` covers 390px Agent entry from production. `app/test/widgets/agent_chat_screen_test.dart` covers the current assistant UI on mobile/desktop surfaces, including entry-family isolation, auto/manual persistence, confirmation cards, Markdown skill visibility, project notes, and the absence of custom JS / supervision / RAG settings.

Known Gaps: Full ToonFlow multi-layer Agent orchestration, external model/vector RAG, ES-style query planning, and custom JS skill execution are intentionally out of scope after v0.4 convergence. Recovering any of them requires a new spec and explicit product decision.

Next Verification: Treat full Agent parity as its own subsystem plan. For the current assistant scope, keep `assistant_*`, `project_notes`, and Agent UI tests green; add new tests only for explicitly accepted assistant behavior.
