...
Desktop Evidence: `app/test/widgets/workbench_screen_test.dart`, `app/test/engine/video_track_test.dart`, and `app/test/engine/compose_episode_test.dart` cover prompt editing, duration editing, per-shot transition/filter metadata editing, selected video candidates, shot audio binding, and compose handoff with NLE metadata carried into `ComposeSegment`.
...
Known Gaps: Full WebAV/NLE parity is not implemented: transition/filter rendering in the platform composer, multi-layer timeline editing, and drag-based clip editing are not proven.
