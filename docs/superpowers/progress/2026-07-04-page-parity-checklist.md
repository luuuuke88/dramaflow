...
Known Gaps: Full WebAV/NLE parity is not implemented: Web transition/filter visual rendering, Web cross-shot dissolve, Web whip-pan rendering, and drag-based multi-layer clip editing are not proven. Current media-library support can attach existing clip assets to a shot as selected candidates, save generated candidates back as clip assets, and add clip assets as editable overlay-layer records; it is still not a full drag-and-drop media bin or freeform timeline.
