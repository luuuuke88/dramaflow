...
Known Gaps: Full WebAV/NLE parity is not implemented: Web transition/filter visual rendering, Web cross-shot dissolve, Web whip-pan rendering, multi-layer timeline editing, and drag-based multi-layer clip editing are not proven. Current drag support reorders storyboard shots in the sequential v1 workbench; it is not a full WebAV-style freeform timeline.
