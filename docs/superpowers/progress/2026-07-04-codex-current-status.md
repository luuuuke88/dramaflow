...
- 最近接力：`16f83d0 docs(progress): record current flutter parity status` 之后继续推进工作台/NLE 素材层属性编辑、复制、自动放层，以及纵向拖拽到占用层时自动下探空层。
...
- `cd app && flutter test`：通过，368 tests。
...
2. 工作台仍不是完整 ToonFlow WebAV/NLE。
   - 已有顺序分镜、候选视频、音频绑定、转场/滤镜 metadata、原生端合成、timeline overlay 渲染、overlay 素材层拖拽/裁剪/属性编辑/复制/自动找空层/纵向拖拽占用层自动下探/吸附/分割/删除/ripple/同轨避让。
