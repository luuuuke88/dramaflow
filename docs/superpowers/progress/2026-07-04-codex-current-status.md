...
- 最近接力：`16f83d0 docs(progress): record current flutter parity status` 之后继续推进工作台/NLE 素材层属性编辑、移动端全屏单列添加/属性编辑/媒体库按播放头添加/批量移动起点/批量改轨/批量裁剪尾部/按播放头切分、透明度编辑/合成、复制、波纹复制、波纹移动、按时间起点可视拉开、按 lane 可视分轨、选中多素材层后按播放头批量切分/复制到播放头/对齐播放头/尾部对齐播放头/中心对齐播放头/裁到播放头/裁开头到播放头/批量删除/批量波纹删除/批量复制/批量波纹复制/批量移动/批量改轨/批量波纹移动/批量裁剪/批量波纹裁剪/整组拖拽平移并后移避让同轨冲突、自动放层、纵向拖拽到占用层时自动下探空层、媒体库按播放头快捷添加到时间线、媒体库 clip 按落点坐标拖放到时间线，以及媒体库拖放起点/尾部接近锚点时自动吸附。
...
- `cd app && flutter test`：通过，414 tests。
...
- 移动端不是空壳：390px widget tests 已覆盖项目、章节、剧本、素材、制作、工作台入口/重排/素材层添加/素材层属性编辑/媒体库按播放头添加/批量移动起点/批量改轨/批量裁剪尾部/按播放头切分、配音、任务、Agent 入口和设置路径。
