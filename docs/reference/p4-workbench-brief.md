...
> 现有实现已补 `o_timelineClip` 式素材层记录、compose metadata handoff、Apple AVFoundation overlay 渲染、Android Media3 overlay 渲染、素材层基础拖拽定位、素材层左右边缘裁剪/缩放、素材层基础吸附、素材层中点分割、素材层按 playhead 任意切点分割、以及时间线素材层移除且保留源素材；吸附可视化/复杂规则、ripple、叠放冲突处理、多轨自由非线编与 WebAV parity 仍按后续切片推进。
