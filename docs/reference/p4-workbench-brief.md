...
> 现有实现已补 `o_timelineClip` 式素材层记录、compose metadata handoff、Apple AVFoundation overlay 渲染、Android Media3 overlay 渲染、以及素材层基础拖拽定位；完整裁剪/缩放型非线编与 WebAV parity 仍按后续切片推进。
