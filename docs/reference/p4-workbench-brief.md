...
> 现有实现已补 `o_timelineClip` 式素材层记录、compose metadata handoff、Apple AVFoundation overlay 渲染、Android Media3 overlay 渲染、素材层基础拖拽定位、左右边缘裁剪/缩放、普通添加时同轨重叠避让、基础吸附、播放头吸附、基础吸附参考线、拖拽/裁剪时同轨重叠避让、中点分割、按 playhead 任意切点分割、时间线素材层移除且保留源素材、素材层波纹删除、素材层尾部波纹裁剪、以及素材层波纹插入；复杂吸附规则、更多复杂 ripple、复杂叠放冲突处理、多轨自由非线编与 WebAV parity 仍按后续切片推进。
